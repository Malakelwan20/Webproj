(function () {
    ...
    if (topNav) {
        topNav.classList.add("has-global-toggle");
        const menuInNav = topNav.querySelector(".menu");
        if (canShowSearch) {
            topNav.insertBefore(searchWrap, menuInNav || topNav.firstChild);
    ...
    function setupGlobalAnimations() {
        ...
        const uniqueTargets = Array.from(document.querySelectorAll(revealSelectors.join(", "))).filter(function (element, index, array) {
            if (!element || element.classList.contains("hidden")) {
                return false;
            }
            if (element.closest(".global-sidebar") || element.closest(".hamburger-menu")) {
                return false;
            }
        ...
        uniqueTargets.forEach(function (element, index) {
            ...
            element.style.setProperty("--motion-delay", Math.min(index * 55, 440) + "ms");
            if (element.matches("img, .community-hero-image, .community-recipe-photo, .uploaded-photo, .profile-photo-preview, .profile-photo-fallback, .meal-photo-icon")) {
                element.classList.add("animated-photo");
            }
            if (element.matches("h1, h2, h3, p, li, .community-recipe-title, .community-recipe-meta, .recipe-steps-title")) {
                element.classList.add("recipe-animated-text");
            }
        });

        if (!("IntersectionObserver" in window)) {
            uniqueTargets.forEach(function (element) {
